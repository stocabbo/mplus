...
function createPlans() {
  const currentHours = Number(document.getElementById('current_hours').value || 0);
  const currentMinutes = Number(document.getElementById('current_minutes').value || 0);
  const targetHours = Number(document.getElementById('target_balance').value);
  const duration = Number(document.getElementById('duration').value);
  const current = (currentHours * 60) + currentMinutes;
  const target = Math.round(targetHours * 60);
  const balanceValues = [currentHours, currentMinutes, targetHours, current, target];
  if (!balanceValues.every(Number.isFinite) || currentHours < 0 || currentMinutes < 0 || currentMinutes > 59
      || current < 0 || current > MAX_BALANCE_MINUTES || target <= current || target > MAX_BALANCE_MINUTES) {
  ...
  if (!Number.isFinite(duration) || duration < 1 || !Number.isInteger(duration)) {
    results.innerHTML = '<p class="planner-error">Inserisci una durata valida di almeno un giorno o una settimana.</p>';
    return;
  }
