...
const exclusionReasons = ['Ferie', 'Smart working', 'Non disponibile'];
...
function buildPlans(totalMinutes, availableDays) {
  ...
  // Non creare righe da 0 minuti quando l'orizzonte contiene più giorni dei
  // minuti ancora da accumulare.
  const usableDays = availableDays.slice(0, totalMinutes);

  const candidates = [
    { count: minimumDays, label: 'Più rapido', description: 'Concentra l’accumulo nel minor numero di giorni.' },
    { count: Math.ceil((minimumDays + usableDays.length) / 2), label: 'Equilibrato', description: 'Bilancia durata e impegno quotidiano.' },
    { count: usableDays.length, label: 'Più leggero', description: 'Distribuisce l’obiettivo su tutti i giorni disponibili.' }
  ].filter((candidate, index, all) => all.findIndex(item => item.count === candidate.count) === index);

  return candidates.map(candidate => {
    const count = candidate.count;
    const dates = usableDays.slice(0, count);
...
function loadExclusions() {
  try {
    ...
    return Array.isArray(saved)
      ? saved.filter(item => isValidISODate(item.date) && exclusionReasons.includes(item.reason))
      : [];
  } catch (error) {
